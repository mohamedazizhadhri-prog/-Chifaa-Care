...
@Component({
  ...
  template: `
    <div class="ad-banner-container">
      ...
      <!-- Left animated banner with dynamic slideshow -->
      <div class="ad-banner ad-banner-left" *ngIf="!isLoading && leftBanners.length > 0">
        <div class="ad-banner-content">
          <div class="pharmacy-slideshow">
            <img 
              *ngFor="let banner of leftBanners; let i = index"
              [src]="banner.imageUrl" 
              [alt]="banner.altText"
              class="slide-image"
              [class.active]="i === currentLeftIndex"
...
export class AdBannerComponent implements OnInit, OnDestroy {
  ...
  handleImageError(event: Event, banner: AdBanner) {
    ...
    // If it's a left banner, try to show the next one
    if (banner.type === 'left') {
      const currentIndex = this.leftBanners.findIndex(b => b.id === banner.id);
      if (currentIndex !== -1 && this.leftBanners.length > 1) {
        this.currentLeftIndex = (currentIndex + 1) % this.leftBanners.length;
      }
    }
  }
  ...
  private processAds(ads: AdBanner[]) {
    ...
    try {
      ...
      if (rightBanner) {
        ...
        // Use first ad as right banner if no right banner is specified
        this.rightBanner = { ...validAds[0], type: 'right' };
      }

      // Start slideshow if we have left banners
      if (this.leftBanners.length > 0) {
        this.startSlideshow();
      } else if (validAds.length > 0) {
        // If we have ads but no left banners, use them all as left banners
        this.leftBanners = [...validAds];
        this.startSlideshow();
      }
    } catch (error) {
  ...
  private loadDefaultAds() {
    ...
    this.isLoading = false;
    this.error = 'Using default ads (server not available)';
  ...
  private startSlideshow() {
    ...
    this.intervalId = setInterval(() => {
      this.currentLeftIndex = (this.currentLeftIndex + 1) % this.leftBanners.length;
