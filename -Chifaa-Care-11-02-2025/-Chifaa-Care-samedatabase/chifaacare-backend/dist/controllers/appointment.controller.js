...
exports.deleteAppointment = exports.updateAppointmentStatus = exports.getDoctorSchedule = exports.getDoctorUpcoming = exports.getDoctorPending = exports.getAppointments = exports.createAppointment = exports.createAppointmentAsDoctor = exports.getAvailableSlots = exports.updateAppointment = void 0;
...
const updateAppointment = async (req, res) => {
    ...
    try {
        const { id } = req.params;
        const { appointmentDate, endTime, reason, notes } = req.body;
        const appointment = await prisma.appointment.findUnique({ where: { id } });
        if (!appointment) {
            return res.status(404).json({ status: 'error', message: 'Appointment not found' });
        }
        // Permission: doctor, patient, or admin
        if (appointment.doctorId !== ((_a = req.user) === null || _a === void 0 ? void 0 : _a.id) &&
            appointment.patientId !== ((_b = req.user) === null || _b === void 0 ? void 0 : _b.id) &&
        ...
        newStart = newStart !== null && newStart !== void 0 ? newStart : new Date(appointment.appointmentDate);
        newEnd = newEnd !== null && newEnd !== void 0 ? newEnd : new Date(appointment.endTime);
        // Basic sanity
        if (newEnd <= newStart) {
        ...
        const conflicting = await prisma.appointment.findFirst({
            where: {
                ...
                status: { in: [appointment_1.APPOINTMENT_STATUS.PENDING, appointment_1.APPOINTMENT_STATUS.CONFIRMED] },
...
const getAvailableSlots = async (req, res) => {
    try {
        ...
        const { date, duration = '30', tz } = req.query;
        if (!doctorId)
            return res.status(400).json({ status: 'error', message: 'doctorId is required' });
        if (!date)
            return res.status(400).json({ status: 'error', message: 'date is required (ISO string)' });
        ...
        try {
            if (doctorProfile === null || doctorProfile === void 0 ? void 0 : doctorProfile.availableHours) {
                ...
                if (Array.isArray(parsed) && parsed.length > 0) {
                    ranges = parsed.map((r) => {
                        ...
                        if (r && typeof r === 'object' && r.start && r.end)
...
const createAppointmentAsDoctor = async (req, res) => {
    var _a, _b;
    try {
        ...
        const appointment = await prisma.appointment.create({
            data: {
                ...
                reason,
                notes,
                status: appointment_1.APPOINTMENT_STATUS.CONFIRMED, // doctor-created defaults to confirmed
            },
            include: {
                patient: { select: { id: true, firstName: true, lastName: true, email: true } },
                doctor: { select: { id: true, firstName: true, lastName: true, email: true } },
...
exports.createAppointmentAsDoctor = createAppointmentAsDoctor;
...
const createAppointment = async (req, res) => {
    try {
        ...
        const message = (error === null || error === void 0 ? void 0 : error.message) || 'An error occurred while creating the appointment';
...
const getAppointments = async (req, res) => {
    try {
        ...
        if (status && (0, appointment_1.isValidAppointmentStatus)(status)) {
        ...
        if (startDate && endDate) {
            whereClause.OR = [
                ...
                },
                {
                    ...
                    AND: [
                        { appointmentDate: { lte: new Date(startDate) } },
...
const getDoctorPending = async (req, res) => {
    ...
    try {
        ...
        const appointments = await prisma.appointment.findMany({
            where: {
                doctorId,
                status: appointment_1.APPOINTMENT_STATUS.PENDING,
            },
            include: {
                patient: {
...
const getDoctorUpcoming = async (req, res) => {
    ...
    try {
        ...
        const appointments = await prisma.appointment.findMany({
            where: {
                doctorId,
                status: appointment_1.APPOINTMENT_STATUS.CONFIRMED,
...
exports.getDoctorUpcoming = getDoctorUpcoming;
/**
