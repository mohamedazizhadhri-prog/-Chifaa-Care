...
export class StripeService {
  ...
  static async confirmPayment(paymentIntentId: string) {
    ...
    try {
      ...
      // If payment succeeded, keep appointment as PENDING - doctor needs to accept
      // Do NOT auto-confirm - this allows doctor approval workflow
      if (paymentIntent.status === 'succeeded') {
        console.log(`Payment succeeded for appointment ${payment.appointmentId} - keeping status as PENDING for doctor approval`);
      }
  ...
  private static async handlePaymentSucceeded(paymentIntent: Stripe.PaymentIntent) {
    try {
      ...
      // Keep appointment as PENDING - doctor needs to manually accept
      // Do NOT auto-confirm appointments after payment
      await prisma.appointment.update({
        where: { id: payment.appointmentId },
        data: {
          // Keep as PENDING for doctor approval
          updatedAt: new Date(),
        },
      });
