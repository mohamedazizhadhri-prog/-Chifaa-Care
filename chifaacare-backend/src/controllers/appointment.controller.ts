...
export const createAppointment = async (req: any, res: Response) => {
  try {
    ...
    console.log('[createAppointment] ID:', appointment.id);
    console.log('[createAppointment] Status:', appointment.status);
    console.log('[createAppointment] Doctor ID:', appointment.doctorId);
    console.log('[createAppointment] Patient ID:', appointment.patientId);
    console.log('[createAppointment] Date:', appointment.appointmentDate);
...
export const getAppointments = async (req: any, res: Response) => {
  try {
    ...
    console.log('[getAppointments] Query params:', { status, startDate, endDate, patientId, doctorId });
    console.log('[getAppointments] User:', { userId, userRole });
    ...
    if (status && isValidAppointmentStatus(status)) {
      ...
      console.log('[getAppointments] Filtering by status:', status);
    }
    ...
    console.log('[getAppointments] Final where clause:', JSON.stringify(whereClause, null, 2));
    ...
    console.log('[getAppointments] Appointments:', appointments.map(a => ({ 
      id: a.id, 
      status: a.status, 
      doctorId: a.doctorId, 
...
export const getDoctorPending = async (req: any, res: Response) => {
  try {
    ...
    const appointments = await prisma.appointment.findMany({
      where: {
        doctorId,
        status: { in: [APPOINTMENT_STATUS.PENDING, APPOINTMENT_STATUS.RESERVED] }, // Support both statuses
      },
      include: {
        patient: {
...
export const acceptAppointment = async (req: any, res: Response) => {
  try {
    ...
    // Verify appointment is pending or reserved
    if (appointment.status !== APPOINTMENT_STATUS.PENDING && appointment.status !== APPOINTMENT_STATUS.RESERVED) {
      return res.status(400).json({
        status: 'error',
        message: `Cannot accept appointment with status ${appointment.status}. Must be PENDING or RESERVED.`,
      });
    }
...
export const rejectAppointment = async (req: any, res: Response) => {
  try {
    ...
    if (appointment.status !== APPOINTMENT_STATUS.PENDING && appointment.status !== APPOINTMENT_STATUS.RESERVED) {
      return res.status(400).json({
        status: 'error',
        message: `Cannot reject appointment with status ${appointment.status}. Must be PENDING or RESERVED.`,
      });
    }
