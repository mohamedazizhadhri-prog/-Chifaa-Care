...
export const getSpecialties = async (req: Request, res: Response) => {
  try {
    // Predefined cancer specialties list
    const cancerSpecialties = [
      ...
      'Head & Neck Cancer',
      'Skin Cancer (Dermatologic Oncology)',
      'Brain & CNS Cancer (Neuro-Oncology)',
    ...
    res.json({ status: 'success', data: cancerSpecialties });
  } catch (err: any) {
...
export const createPatient = async (req: Request, res: Response) => {
  try {
    const { email, firstName, lastName, phone, dateOfBirth, gender, address, password = 'Patient@123' } = req.body;
    ...
    res.status(201).json({ status: 'success', data: patientData });
  } catch (err: any) {
    console.error('Error creating patient:', err);
    res.status(500).json({ status: 'error', message: err.message || 'Failed to create patient' });
  }
};
...
export const deletePatient = async (req: Request, res: Response) => {
  try {
    ...
    if (user.role !== 'PATIENT') {
      return res.status(400).json({ status: 'error', message: 'User is not a patient' });
    }
    ...
    if (appointmentsCount > 0) {
      return res.status(400).json({ 
        status: 'error', 
        message: `Cannot delete patient with ${appointmentsCount} associated appointment(s). Please archive appointments first.` 
      });
    }
    ...
    res.status(500).json({ status: 'error', message: err.message || 'Failed to delete patient' });
  }
};
...
export const deleteDoctor = async (req: Request, res: Response) => {
  try {
    ...
    if (user.role !== 'DOCTOR') {
      return res.status(400).json({ status: 'error', message: 'User is not a doctor' });
    }
    ...
    if (appointmentsCount > 0) {
      return res.status(400).json({ 
        status: 'error', 
        message: `Cannot delete doctor with ${appointmentsCount} associated appointment(s). Please reassign or cancel appointments first.` 
      });
    }

    // Delete in transaction: first doctor profile, then user
    ...
    res.status(500).json({ status: 'error', message: err.message || 'Failed to delete doctor' });
  }
};
