...
export class PatientListComponent implements OnInit {
  ...
  loadPendingBookings() {
    ...
    // Normalize role check - handle both 'doctor' and 'DOCTOR'
    ...
    // Load appointments with PENDING or RESERVED status (these need doctor approval)
    // Note: Database might have RESERVED status instead of PENDING
    console.log('[loadPendingBookings] Fetching appointments for doctor:', currentUser.id);
    ...
    this.appointmentService.getAppointments({ 
      ...
      next: (appts: AppointmentWithRelations[]) => {
        ...
        console.log('Number of pending appointments:', appts?.length || 0);
        
        if (!appts || appts.length === 0) {
          console.log('No PENDING appointments found, trying RESERVED status...');
          ...
          this.appointmentService.getAppointments({ 
            ...
            next: (reservedAppts: AppointmentWithRelations[]) => {
              console.log('Received RESERVED appointments:', reservedAppts);
              if (!reservedAppts || reservedAppts.length === 0) {
                console.log('No RESERVED appointments found either');
                this.pendingBookings = [];
              } else {
                this.pendingBookings = reservedAppts.map(a => ({
                  id: a.id,
                ...
                console.log('Final pendingBookings (from RESERVED):', this.pendingBookings);
              }
            },
            error: (err) => {
              console.error('Failed to load RESERVED bookings:', err);
              this.pendingBookings = [];
            }
          });
          return;
          ...
          this.pendingBookings = appts.map(a => {
            ...
            console.log('Mapped booking:', booking);
            return booking;
          });
          console.log('Final pendingBookings:', this.pendingBookings);
        }
      },
      error: (err) => {
    ...
    // Load confirmed consultations (these were already accepted)
    this.appointmentService.getAppointments({ 
      doctorId: currentUser.id,
      status: 'CONFIRMED' as any 
    }).subscribe({
      next: (appts: AppointmentWithRelations[]) => {
        console.log('Received confirmed appointments:', appts);
        console.log('Number of confirmed appointments:', appts?.length || 0);
        
        if (!appts || appts.length === 0) {
          console.log('No confirmed appointments found');
          this.confirmedConsultations = [];
        } else {
          this.confirmedConsultations = appts.map(a => ({
            id: a.id,
            patientName: `${a.patient?.firstName || ''} ${a.patient?.lastName || ''}`.trim() || 'Patient',
            consultationType: a.consultationType || 'Consultation',
            preferredDate: this.formatDisplayDate(a.appointmentDate),
          ...
          console.log('Final confirmedConsultations:', this.confirmedConsultations);
        }
      },
      error: (err) => {
        console.error('Failed to load confirmed appointments:', err);
        console.error('Error details:', err.error || err.message);
        this.confirmedConsultations = [];
      }
    });
  ...
  acceptBooking(bookingId: string) {
    ...
    this.appointmentService.acceptAppointment(bookingId).subscribe({
      next: (updated) => {
        ...
        alert('Appointment accepted successfully! The patient will be notified.');
      },
      error: (err) => {
        console.error('Failed to accept booking:', err);
        const message = err?.error?.message || err?.message || 'Failed to accept booking';
        alert(`Error: ${message}`);
      }
    });
  }
  ...
  refuseBooking(bookingId: string) {
    ...
    this.appointmentService.rejectAppointment(bookingId, reason || undefined).subscribe({
      next: (result) => {
        console.log('Appointment rejected:', result);
        this.pendingBookings = this.pendingBookings.filter(b => b.id !== bookingId);
        alert('Appointment rejected. A refund has been initiated for the patient.');
      },
      error: (err) => {
        console.error('Failed to reject booking:', err);
        const message = err?.error?.message || err?.message || 'Failed to reject booking';
        alert(`Error: ${message}`);
      }
    });
  }
