...
export class AppointmentService {
  ...
  /**
   * Doctor accepts a pending appointment (verifies payment and adds to calendar)
  ...
  acceptAppointment(appointmentId: string): Observable<AppointmentWithRelations> {
    return this.http.post<any>(
      `${this.apiUrl}/${appointmentId}/accept`,
      {}
    ).pipe(
      map((resp) => (resp?.data?.appointment || resp) as AppointmentWithRelations),
      catchError(error => {
  ...
  /**
   * Doctor rejects a pending appointment (initiates refund)
