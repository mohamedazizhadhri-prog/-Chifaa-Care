...
export class DoctorDoctorMessagesComponent implements OnInit, OnDestroy {
  ...
  private conversationRefreshInterval = 10000; // 10 seconds - less aggressive
  ...
  private isLoadingMessages = false; // Prevent concurrent message loads
  ...
  private initializeComponent(): void {
    console.log('[Doctor Chat] Initializing component for user:', this.currentUser?.id);
    
    // IMPORTANT: Setup socket connection FIRST, then listeners
    if (this.currentUser?.id) {
      console.log('[Doctor Chat] Connecting socket...');
      this.socketService.connect(this.currentUser.id);
      
      // Wait a moment for socket to connect before setting up listeners
      setTimeout(() => {
  ...
  private loadMessages(chatId: string): void {
    if (!this.currentUser?.id || this.isLoadingMessages) return;
    
    this.isLoadingMessages = true;
    console.log('[Doctor Chat] Loading messages for chat:', chatId);
    ...
    this.messageService.getThread(this.currentUser.id, chatId).subscribe({
      next: (response) => {
        if (response.status === 'success') {
          ...
          console.log('[Doctor Chat] Loaded', this.currentMessages.length, 'messages');
          this.scrollToBottom();
        }
  ...
  sendMessage(): void {
    ...
    const recipientId = this.selectedChatId;
    this.newMessage = ''; // Clear input immediately
    
    // Track when we sent this message
    this.lastMessageSentTime = Date.now();
    
    // Optimistically add the message to UI immediately with proper sender info
    ...
    const chat = this.conversations.find(c => c.id === recipientId);
    if (chat) {
    ...
    this.messageService.sendMessage(
      ...
      next: (response) => {
        if (response.status === 'success') {
          ...
          console.log('[Doctor Chat] Message sent successfully');
        }
      },
      error: (error) => {
  ...
  cancelCall(): void {
    ...
    if (this.dialing && this.currentUser?.id && this.selectedChatId) {
