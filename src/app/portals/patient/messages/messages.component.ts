...
@Component({
      ...
      <div class="messages-layout">
        ...
        <aside class="chat-sidebar card lift">
          ...
          <div *ngIf="showDoctorPicker" class="doctor-picker card" style="padding: 8px; border: 1px solid #e2e8f0; border-radius: 8px; margin-bottom: 8px;">
            ...
            <div *ngFor="let d of doctors" (click)="startChatWithDoctor(d)" style="display: flex; align-items: center; gap: 8px; padding: 8px; border-radius: 8px; cursor: pointer; transition: background-color 0.2s;" 
                 [style.background]="hoveredDoctorId === d.id ? '#f8fafc' : 'transparent'"
                 (mouseenter)="hoveredDoctorId = d.id"
                 (mouseleave)="hoveredDoctorId = null">
              <div class="chat-avatar" style="background: #22c55e; width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-size: 14px;">
              ...
              <div style="flex: 1; min-width: 0;">
                ...
                <div style="font-size: 0.8rem; color: #64748b; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                  {{ d.doctorProfile?.specialization || 'Doctor' }}
                </div>
              </div>
            </div>
          </div>
          <!-- Conversation List -->
          <div class="chat-list">
            <div *ngIf="filteredChats.length === 0" class="no-conversations">
              <i class="fa-solid fa-inbox"></i>
              <p>No conversations yet</p>
              <small>Click "New chat" to message a doctor</small>
            </div>
            <div *ngFor="let chat of filteredChats" 
              ...
              <div class="chat-avatar" [style.background]="chat.isOnline ? '#22c55e' : '#94a3b8'">
                <i class="fa-solid fa-user-doctor"></i>
              </div>
              <div class="chat-info">
        ...
        <section class="chat-window card lift" *ngIf="selectedChat">
          ...
          <div class="chat-header">
            ...
            <div class="chat-actions">
              ...
              <button 
                ...
                style="width: 40px; height: 40px; border-radius: 50%; background: linear-gradient(135deg, #10b981 0%, #059669 100%); color: white; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.3s ease;">
              ...
              <button 
                ...
                [disabled]="inCall || dialing"
                style="width: 40px; height: 40px; border-radius: 50%; background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); color: white; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.3s ease; margin-left: 8px;">
                <i class="fa-solid fa-video"></i>
              </button>
              
              <!-- Call status indicator -->
              <span *ngIf="dialing" class="call-indicator dialing" style="margin-left: 8px;">
                <i class="fa-solid fa-phone fa-shake"></i> Calling...
              </span>
              <span *ngIf="inCall" class="call-indicator active" style="margin-left: 8px;">
                <i class="fa-solid fa-phone"></i> In Call
              </span>
              
              <!-- Hang up button when dialing (ringing) -->
              <button 
                *ngIf="dialing"
                class="btn btn-danger btn-hang-up-ringing" 
                (click)="cancelCall()"
                title="Cancel call"
                style="margin-left: 8px; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center;">
                <i class="fa-solid fa-phone-slash"></i>
              </button>
              
              <button class="btn btn-outline" (click)="bookAppointment()" style="margin-left: 8px;">
          ...
          <!-- Call Panel -->
          <div class="call-panel card" *ngIf="inCall" style="margin: 12px; padding: 12px; background: white; border-radius: 8px; border: 1px solid #e2e8f0;">
            <div class="call-header" style="display:flex; align-items:center; justify-content: space-between; margin-bottom: 8px;">
              <strong>Call with {{ selectedChat.doctorName }}</strong>
              <span class="badge" [class.badge-video]="mediaType==='video'" [class.badge-audio]="mediaType==='audio'" style="padding: 2px 8px; border-radius: 999px; font-size: 0.75rem;">
                {{ mediaType | uppercase }}
              </span>
            </div>
            <div class="videos" [class.audio-only]="mediaType==='audio'" style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
              <video #localVideo autoplay playsinline muted style="width: 100%; background: #000; border-radius: 8px; min-height: 160px;"></video>
              <video #remoteVideo autoplay playsinline [muted]="false" style="width: 100%; background: #000; border-radius: 8px; min-height: 160px;"></video>
            </div>
            <div class="call-actions" style="display:flex; justify-content:center; margin-top: 8px;">
          ...
          <!-- Messages Area -->
          <div class="messages-area" #messagesContainer [class.with-call]="inCall">
  ...
  styles: [`
    ...
    /* Hang up button when ringing (compact version for header) */
    .btn-hang-up-ringing {
      background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%) !important;
      box-shadow: 0 2px 8px rgba(239, 68, 68, 0.3) !important;
      transition: all 0.3s ease !important;
    ...
    .btn-hang-up-ringing:hover:not(:disabled) {
      background: linear-gradient(135deg, #dc2626 0%, #b91c1c 100%) !important;
      box-shadow: 0 4px 12px rgba(239, 68, 68, 0.4) !important;
      transform: scale(1.05) !important;
    }
    
    .btn-hang-up-ringing i {
      font-size: 1rem;
    }
...
export class PatientMessagesComponent implements OnInit, OnDestroy {
  ...
  cancelCall() {
    // Cancel outgoing call while dialing/ringing
    if (this.dialing && this.currentPatientId && this.selectedOtherUserId) {
      this.socket.emit('call:cancel', { fromUserId: this.currentPatientId, toUserId: this.selectedOtherUserId });
      this.cleanupCall();
    }
  }
