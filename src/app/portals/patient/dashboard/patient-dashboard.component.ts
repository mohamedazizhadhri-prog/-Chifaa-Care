        ...
        <div class="card lift" style="grid-column: span 2;">
          ...
          <div *ngIf="nextAppointment; else noAppointment" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;">
            ...
            <div style="display:flex;gap:8px;flex-wrap:wrap;">
              ...
              <button class="btn btn-blue-outline" (click)="toggleReschedule()">
                <i class="fa-solid fa-calendar"></i> Reschedule
              </button>
            </div>
            <div *ngIf="showReschedule" class="card" style="padding:12px;margin-top:8px;border:1px dashed #cbd5e1;border-radius:12px;">
              <div style="display:flex;gap:8px;align-items:end;flex-wrap:wrap;">
                <label>Choose date<br>
                  <input type="date" [(ngModel)]="rescheduleDate" (change)="onRescheduleDateChange()" style="padding:.6rem 1rem;border:1px solid #e5e7eb;border-radius:12px;" />
                </label>
                <label>Choose slot<br>
                  <select [(ngModel)]="selectedSlotIndex" style="padding:.6rem 1rem;border:1px solid #e5e7eb;border-radius:12px;min-width:220px;">
                    <option [ngValue]="null" disabled>Select a time</option>
                    <option *ngFor="let s of availableSlots; index as i" [ngValue]="i">{{ formatTime(s.start) }} - {{ formatTime(s.end) }}</option>
                  </select>
                </label>
                <button class="btn btn-green" (click)="submitReschedule()" [disabled]="selectedSlotIndex === null || availableSlots.length === 0">Confirm</button>
                <button class="btn btn-blue-outline" (click)="toggleReschedule()">Cancel</button>
              </div>
              <div class="muted" *ngIf="rescheduleMessage" style="margin-top:6px;">{{ rescheduleMessage }}</div>
            </div>
          </div>
...
export class PatientDashboardComponent implements OnInit {
  ...
  getDisplayName(): string {
    ...
    // Return only the first name, capitalized
  ...
  toggleReschedule(): void {
    ...
    if (this.showReschedule) {
      ...
      // default date = current appointment date
      if (this.nextAppointment?.appointmentDate) {
        const d = new Date(this.nextAppointment.appointmentDate);
        const yyyy = d.getFullYear();
        const mm = String(d.getMonth() + 1).padStart(2, '0');
        const dd = String(d.getDate()).padStart(2, '0');
        this.rescheduleDate = `${yyyy}-${mm}-${dd}`;
        this.onRescheduleDateChange();
      }
    }
  }

  onRescheduleDateChange(): void {
    if (!this.nextAppointment?.doctorId || !this.rescheduleDate) return;
    const date = new Date(this.rescheduleDate + 'T00:00:00');
    this.rescheduleMessage = 'Loading available slots…';
    this.appointmentService.getAvailableSlots(this.nextAppointment.doctorId, date, 30).subscribe({
      next: (slots) => {
        this.availableSlots = slots || [];
        this.selectedSlotIndex = null;
        this.rescheduleMessage = this.availableSlots.length ? '' : 'No available slots for selected date.';
      },
      error: (err) => {
        console.error('Error loading slots', err);
        this.rescheduleMessage = 'Failed to load slots.';
      }
    });
  }

  submitReschedule(): void {
    if (!this.nextAppointment || this.selectedSlotIndex === null) return;
    const slot = this.availableSlots[this.selectedSlotIndex];
    if (!slot) return;
    this.rescheduleMessage = 'Rescheduling…';
    this.appointmentService.rescheduleAppointment(
      this.nextAppointment.id,
      slot.start,
      slot.end,
      'Patient requested reschedule'
    ).subscribe({
      next: (updated) => {
        this.rescheduleMessage = 'Rescheduled successfully.';
        this.nextAppointment = updated as any;
        this.showReschedule = false;
      },
      error: (err) => {
        console.error('Reschedule failed', err);
        this.rescheduleMessage = 'Failed to reschedule. Please try another slot.';
      }
    });
  }
