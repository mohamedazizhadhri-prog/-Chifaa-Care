...
// Using shared ConsultationType from models
...
export class BookConsultationComponent implements OnInit {
  ...
  submitBooking(): void {
    ...
    if (!user || user.role !== 'patient') {
      alert('Please log in as a patient to book a consultation.');
      return;
    }

    const dateStr: string = this.bookingForm.get('date')?.value;
    const timeStr: string = this.bookingForm.get('time')?.value;
    const reason: string = this.bookingForm.get('reason')?.value;
    const type: ConsultationType = this.bookingForm.get('consultationType')?.value as ConsultationType;

    // Compose ISO datetimes
