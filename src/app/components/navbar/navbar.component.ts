    ...
    <nav class="navbar" [class.scrolled]="isScrolled">
      <div class="navbar-container">
        ...
        <div class="navbar-logo">
          <img src="assets/images/chifaacare-logo.png" alt="ChifaaCare logo" />
        </div>
        ...
        <ng-template #userBadgeTpl>
              ...
              <div class="nav-id-badge">
                ...
                <div class="id-lines">
                  <div class="id-name">{{ getFirstName(currentUser?.name) }}</div>
...
export class NavbarComponent implements OnInit {
  ...
  overlayRole: 'patient' | 'doctor' | 'clinic' | 'admin' = 'patient';
  overlayName = '';
  ...
  ngOnInit() {
    ...
    // Listen for custom event from hero component to show login modal
    document.addEventListener('showLoginModal', (event: any) => {
      if (event.detail?.mode === 'signup') {
        // Switch to signup mode before opening modal
        this.authModal?.switchMode('signup');
      }
      this.openLoginModal();
    });
  }
  ...
  onAuthAnimation(evt: { role: 'patient'|'doctor'|'clinic'|'admin'; name: string; durationMs: number }) {
  ...
  getRoleAvatar(role?: 'patient'|'doctor'|'clinic'|'admin'): string {
    // Prefer PNG if available (as per user's provided assets), otherwise fall back to SVG
    if (role === 'doctor') return 'assets/avatars/doc.png';
    if (role === 'admin') return 'assets/avatars/clinic.png';
    if (role === 'clinic') return 'assets/avatars/clinic.png';
    return 'assets/avatars/pat.png';
  }

  onAvatarError(ev: Event, role?: 'patient'|'doctor'|'clinic'|'admin') {
    const img = ev.target as HTMLImageElement;
    // Fallback to SVG if PNG missing
    if (role === 'doctor') img.src = 'assets/avatars/doc.svg';
    else if (role === 'clinic' || role === 'admin') img.src = 'assets/avatars/clinic.svg';
    else img.src = 'assets/avatars/pat.svg';
  }
  ...
  logout() {
    try {
      ...
      // Clear any overlay states
      this.showBadgeOverlay = false;
      this.overlayScanner = false;
      this.overlaySlideUp = false;
      ...
      this.router.navigate(['/home']).then(() => {
        console.log('Successfully navigated to home after logout');
      }).catch(error => {
      ...
      // Fallback: force logout and navigation
      this.auth.logout();
      window.location.href = '/home';
    }
  }
