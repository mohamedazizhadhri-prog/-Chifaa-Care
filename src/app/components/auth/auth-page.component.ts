...
@Component({
  ...
  styles: [`
    ...
    .custom-select i { color: #7f8c8d; font-size: 0.9rem; transition: transform .2s; }
    .custom-select-wrapper:has(.dropdown-panel) .custom-select i { transform: rotate(180deg); }
    ...
    .search-box input:focus { border-color: rgba(52,152,219,.6); box-shadow: 0 0 0 3px rgba(52,152,219,.1); }
    ...
    .option-item.selected { background: #e3f2fd; color: #2196F3; font-weight: 500; }
...
export class AuthPageComponent {
  ...
  async ngOnInit() {
    // Try to fetch the canonical specializations from the server (public endpoint)
    try {
      const resp: any = await fetch(`${this.API}/auth/specializations`).then(r => r.json());
      const list = Array.isArray(resp) ? resp : resp?.data || resp?.specializations;
      if (Array.isArray(list) && list.length) this.specialties = list;
    } catch (e) {
      // fallback to local list already assigned
      console.warn('Could not load specializations from server; using local list', e);
    }
  }
