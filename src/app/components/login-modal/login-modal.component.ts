...
type UserRole = 'patient' | 'doctor' | 'clinic' | 'admin';
...
export class LoginModalComponent implements OnInit {
  ...
  constructor(private router: Router, private authService: AuthService, private adminService: AdminService, private http: HttpClient) {}
  ...
  async ngOnInit(): Promise<void> {
    ...
    // Prefer the public specializations endpoint (no auth required).
    // Fall back to admin endpoint, then to local list.
    try {
      const resp = await this.http.get<any>(`${this.API}/auth/specializations`).toPromise();
      const list = Array.isArray(resp) ? resp : resp?.data || resp?.specializations;
      if (Array.isArray(list) && list.length) {
        this.doctorSpecialties = list;
        return;
      }
    } catch (e) {
      // ignore and try admin endpoint next
    }

    try {
      const resp: any = await firstValueFrom(this.adminService.getSpecialties());
      const list = Array.isArray(resp) ? resp : resp?.data;
      if (Array.isArray(list) && list.length > 0) this.doctorSpecialties = list;
    } catch (err) {
      console.warn('Could not fetch specialties from admin API, using local list.', err);
    }
  }
  ...
  private async handleSignup(): Promise<void> {
    ...
    try {
      console.log('Submitting signup payload to AuthService:', signupPayload);
      ...
      // Try to extract useful server-side validation messages
      const serverMessage = error?.error?.message || error?.message || (error?.error ? JSON.stringify(error.error) : null);
      const validationErrors = error?.error?.errors || error?.error?.validation || null;
      if (validationErrors && typeof validationErrors === 'object') {
        // Concatenate field errors if present
        const fieldMessages = Object.entries(validationErrors).map(([k, v]) => `${k}: ${Array.isArray(v) ? v.join(', ') : v}`).join('\n');
        this.errorMessage = `${serverMessage || 'Registration failed.'}\n${fieldMessages}`;
      } else {
        this.errorMessage = serverMessage || 'Registration failed. Please try again.';
      }
      throw error;
    }
  }
  ...
  private startBadgeAnimation(role: UserRole, name: string) {
    ...
    setTimeout(() => {
      // Ensure role is lowercase for routing
      const roleRoute = role.toLowerCase();
      this.router.navigate(['/', roleRoute, 'dashboard']);
