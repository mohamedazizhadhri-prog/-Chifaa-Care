import { Component, OnInit, ChangeDetectorRef } from '@angular/core';
import { CommonModule } from '@angular/common';
import { RouterLink, RouterOutlet, RouterLinkActive, Router, NavigationEnd } from '@angular/router';
import { AuthService, User } from '../../../services/auth.service';
import { AvatarComponent } from '../avatar/avatar.component';
import { ProfileService } from '../../../services/profile.service';

@Component({
  selector: 'app-portal-layout',
  standalone: true,
  imports: [CommonModule, RouterLink, RouterLinkActive, RouterOutlet, AvatarComponent],
  template: `
    <div class="portal">

      <div class="portal-shell container">
        <aside class="portal-sidebar">
          <div class="user-card">
            <app-avatar [src]="avatarSrc" [name]="displayName" [size]="56" shape="circle" [badge]="isDoctor ? 'Dr' : null" [badgeColor]="isDoctor ? '#3b82f6' : '#22c55e'"></app-avatar>
            <div class="user-meta">
              <div class="name">{{ getFirstName(displayName) || 'User' }}</div>
              <div class="role" [class.doctor]="isDoctor" [class.patient]="isPatient" [class.clinic]="isClinic">{{ isDoctor ? 'Doctor' : isPatient ? 'Patient' : isClinic ? 'Clinic Admin' : isAdmin ? 'Admin' : 'User' }}</div>
            </div>
          </div>
          <nav class="menu">
            <a routerLink="dashboard" routerLinkActive="active" class="menu-item"><i class="fa-solid fa-gauge-high"></i> <span>Dashboard</span></a>
            
            <!-- Admin-only navigation -->
            <ng-container *ngIf="isAdmin">
              <a routerLink="users" routerLinkActive="active" class="menu-item"><i class="fa-solid fa-users-gear"></i> <span>Users</span></a>
              <a routerLink="roles" routerLinkActive="active" class="menu-item"><i class="fa-solid fa-user-shield"></i> <span>Roles & Permissions</span></a>
              <a routerLink="clinics" routerLinkActive="active" class="menu-item"><i class="fa-solid fa-hospital"></i> <span>Clinics</span></a>
              <a routerLink="doctors" routerLinkActive="active" class="menu-item"><i class="fa-solid fa-user-doctor"></i> <span>Doctors</span></a>
              <a routerLink="patients" routerLinkActive="active" class="menu-item"><i class="fa-regular fa-user"></i> <span>Patients</span></a>
              <a routerLink="appointments" routerLinkActive="active" class="menu-item"><i class="fa-solid fa-calendar"></i> <span>Appointments</span></a>
              <a routerLink="audit-logs" routerLinkActive="active" class="menu-item"><i class="fa-solid fa-list-check"></i> <span>Audit Logs</span></a>
              <a routerLink="reports" routerLinkActive="active" class="menu-item"><i class="fa-solid fa-chart-line"></i> <span>Reports</span></a>
              <a routerLink="settings" routerLinkActive="active" class="menu-item"><i class="fa-solid fa-gear"></i> <span>Settings</span></a>
            </ng-container>

            <!-- Patient-only navigation -->
            <ng-container *ngIf="isPatient">
              <a routerLink="book-consultation" routerLinkActive="active" class="menu-item"><i class="fa-solid fa-calendar-check"></i> <span>Book Consultation</span></a>
              <a routerLink="treatment-plan" routerLinkActive="active" class="menu-item"><i class="fa-solid fa-prescription-bottle-medical"></i> <span>Treatment Plan</span></a>
              <a routerLink="medical-records" routerLinkActive="active" class="menu-item"><i class="fa-regular fa-file-medical"></i> <span>Medical Records</span></a>
              <a routerLink="partner-clinics" routerLinkActive="active" class="menu-item"><i class="fa-solid fa-hospital"></i> <span>Partner Clinics</span></a>
            </ng-container>
            
            <!-- Doctor-only navigation -->
            <ng-container *ngIf="isDoctor">
              <a routerLink="calendar" routerLinkActive="active" class="menu-item"><i class="fa-solid fa-calendar-days"></i> <span>Calendar</span></a>
              <a routerLink="patients" routerLinkActive="active" class="menu-item"><i class="fa-regular fa-user"></i> <span>Patients</span></a>
              <a routerLink="consultations" routerLinkActive="active" class="menu-item"><i class="fa-solid fa-video"></i> <span>Consultations</span></a>
              <a routerLink="treatment-management" routerLinkActive="active" class="menu-item"><i class="fa-solid fa-clipboard-list"></i> <span>Treatment Management</span></a>
            </ng-container>
            
            <!-- Clinic-only navigation -->
            <ng-container *ngIf="isClinic">
              <a routerLink="patients" routerLinkActive="active" class="menu-item"><i class="fa-solid fa-users"></i> <span>Manage Patients</span></a>
              <a routerLink="doctors" routerLinkActive="active" class="menu-item"><i class="fa-solid fa-user-doctor"></i> <span>Manage Doctors</span></a>
              <a routerLink="appointments" routerLinkActive="active" class="menu-item"><i class="fa-solid fa-calendar"></i> <span>Appointments</span></a>
              <a routerLink="reports" routerLinkActive="active" class="menu-item"><i class="fa-solid fa-chart-bar"></i> <span>Reports</span></a>
            </ng-container>
            
            <!-- Common navigation -->
            <a routerLink="ai-reports" routerLinkActive="active" class="menu-item"><i class="fa-solid fa-wave-square"></i> <span>AI Reports</span></a>
            <a routerLink="messages" routerLinkActive="active" class="menu-item"><i class="fa-regular fa-comments"></i> <span>Messages</span></a>
            <a routerLink="settings" routerLinkActive="active" class="menu-item"><i class="fa-solid fa-gear"></i> <span>Settings</span></a>
          </nav>
        </aside>

        <main class="portal-content">
          <router-outlet></router-outlet>
        </main>
        
        <div class="portal-spacer"></div>
      </div>
    </div>
  `,
  styles: [`
    :host{display:block}
    .portal-topbar{ position: sticky; top: 0; z-index: 10; background: #fff; border-bottom: 1px solid #eef2f7; }
    .portal-shell{ display: flex; gap: 48px; align-items: start; padding: 16px 16px 16px 12px; min-height: calc(100vh - var(--nav-h)); max-width: 1400px; margin: 0 auto; }
    .portal-sidebar{ position: sticky; top: 72px; align-self: start; width: 240px; flex-shrink: 0; }
    .portal-content{ background: #fff; border-radius: var(--radius); padding: 16px; box-shadow: var(--shadow-sm); border: 1px solid rgba(0,0,0,0.04); }
    .user-card{ display:flex; align-items:center; gap:12px; padding:12px; margin-bottom:8px; background: #ffffff; border: 1px solid #eef2f7; border-radius: 12px; box-shadow: var(--shadow-sm); }
    .user-meta .name{ font-weight: 600; color:#0f172a; }
    .user-meta .role{ font-size: 12px; color:#64748b; margin-top:2px; display:inline-block; padding:2px 6px; border-radius:999px; border:1px solid #e5e7eb; }
    .user-meta .role.doctor{ color:#1d4ed8; border-color:#bfdbfe; background:#eff6ff; }
    .user-meta .role.patient{ color:#15803d; border-color:#bbf7d0; background:#f0fdf4; }
    .user-meta .role.clinic{ color:#7c3aed; border-color:#c4b5fd; background:#f3f0ff; }
    .user-meta .role.admin{ color:#111827; border-color:#d1d5db; background:#f9fafb; }
    .menu { display:grid; gap:.4rem; }
    .menu-item { display:flex; align-items:center; gap:.6rem; padding:.7rem .9rem; border-radius:12px; color:#2b3a49; text-decoration:none; transition:background .2s, transform .15s; }
    .menu-item i{ color: var(--c-blue); }
    .menu-item:hover{ background: rgba(52,152,219,.06); transform: translateY(-1px); }
    .menu-item.active{ background: linear-gradient(135deg, rgba(46,204,113,.12), rgba(52,152,219,.12)); box-shadow: var(--shadow-sm); }

    @media (max-width: 1024px){
      .portal-shell{ grid-template-columns: 140px 1fr; gap: 20px; }
    }
    @media (max-width: 768px){
      .portal-shell{ grid-template-columns: 1fr; padding: 16px; }
      .portal-sidebar{ position: static; }
    }
  `]
})
export class PortalLayoutComponent implements OnInit {
  currentUser: User | null = null;
  isPatient = false;
  isDoctor = false;
  isClinic = false;
  isAdmin = false;
  displayName = '';
  avatarSrc: string | null = null;

  constructor(private router: Router, private authService: AuthService, private profileService: ProfileService, private cdr: ChangeDetectorRef) {
    // Remember last visited patient/doctor sub-route to improve /account redirect UX
    this.router.events.subscribe(ev => {
      if (ev instanceof NavigationEnd) {
        const url = ev.urlAfterRedirects || ev.url;
        if (url.startsWith('/patient/')) {
          sessionStorage.setItem('lastPatientRoute', url);
        } else if (url.startsWith('/doctor/')) {
          sessionStorage.setItem('lastDoctorRoute', url);
        } else if (url.startsWith('/clinic/')) {
          sessionStorage.setItem('lastClinicRoute', url);
        } else if (url.startsWith('/admin/')) {
          sessionStorage.setItem('lastAdminRoute', url);
        }
      }
    });
  }

  ngOnInit() {
    this.currentUser = this.authService.getCurrentUser();
    if (this.currentUser) {
      this.isPatient = this.currentUser.role === 'patient';
      this.isDoctor = this.currentUser.role === 'doctor';
      this.isClinic = this.currentUser.role === 'clinic';
      this.isAdmin = this.currentUser.role === 'admin';
      this.displayName = (this.currentUser as any).name || '';
    }
    // Fetch profile image from backend
    this.profileService.getMyProfile().subscribe({
      next: (res: any) => {
        console.log('Profile API Response:', res);
        const data = res?.data || res;
        console.log('Profile Data:', data);
        
        const img = data?.profileImage || data?.user?.profileImage;
        if (img) this.avatarSrc = img;
        
        // Always update displayName from profile if available (prioritize profile data)
        // Try different possible locations for firstName/lastName
        const fn = data?.firstName || data?.user?.firstName || '';
        const ln = data?.lastName || data?.user?.lastName || '';
        console.log('FirstName:', fn, 'LastName:', ln);
        
        const fullName = `${fn} ${ln}`.trim();
        
        if (fullName) {
          console.log('Setting displayName to:', fullName);
          this.displayName = fullName;
          
          // Also update the stored user data in localStorage and AuthService so it persists
          if (this.currentUser) {
            this.currentUser = { ...this.currentUser, name: fullName } as any;
            localStorage.setItem('current_user', JSON.stringify(this.currentUser));
            // Update the AuthService's BehaviorSubject so all components get the updated user
            this.authService['currentUserSubject'].next(this.currentUser);
          }
          
          // Trigger change detection to update the view
          this.cdr.detectChanges();
        } else {
          console.log('No firstName/lastName found, keeping displayName as:', this.displayName);
        }
      },
      error: (err) => {
        console.error('Profile fetch error:', err);
      }
    });
  }

  getFirstName(fullName?: string): string {
    console.log('getFirstName called with:', fullName);
    if (!fullName) return '';
    
    // If the name contains an @ symbol, it's likely an email - extract the username part
    if (fullName.includes('@')) {
      fullName = fullName.split('@')[0];
    }
    
    // Split the full name and return only the first part
    const firstName = fullName.split(' ')[0];
    
    // Capitalize first letter if it's lowercase
    const result = firstName.charAt(0).toUpperCase() + firstName.slice(1).toLowerCase();
    console.log('getFirstName returning:', result);
    return result;
  }
}
